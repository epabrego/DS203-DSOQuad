...
const FPGAosTbase FPGA_TbaseOS[20]={
   //.Psc  .Arr  .OS_Size  .Limit    
      {1-1,40-1,  60000, 5 },      //1S
      {1-1,20-1,  60000, 5 },      //500MS
      {1-1,8-1,   60000, 5 },      //200MS
      {1-1,4-1,   60000, 5 },      //100MS  
      {1-1,2-1,   60000, 5 },      //50MS 
      {1-1,1-1,   48000, 5 },      //20MS
      {1-1,1-1,   24000, 5 },      //10MS 
      {1-1,1-1,   12000, 5 },      //5mS
      {1-1,1-1,   4800 , 5 },      //2MS
      {1-1,1-1,   2400 , 5 },      //1MS  
      {1-1,1-1,   1200 , 4 },      //500uS
      {1-1,1-1,   480  , 5 },      //200uS
      {1-1,1-1,   240  , 4 },      //100uS  
      {1-1,1-1,   120  , 3 },      //50uS 
      {1-1,1-1,   48   , 3 },      //20uS  
...
void Update_Base(void) 
{
  ...
  if(ChartLogic()){
    ...
    __Set(32+0x0F,0x00);                                         //shut off FPGA oversampling                      
    __Set(32+0x10,0x00);                       
    __Set_Param(8,0x96);                         
    ...
    if(OSBufferLogic()){                             
      ...
      if((FPGAsubVer)&&(FPGAosFlag)){
        ...
        if(OS_Range>FPGA_TbaseOS[_T_base].Limit)Tmp=FPGA_TbaseOS[_T_base].Limit;
          else Tmp=OS_Range;
        FPGA_OS_Size=FPGA_TbaseOS[_T_base].OS_Size>>Tmp;
        FPGA_ARR=((FPGA_TbaseOS[_T_base].Arr+1)<<Tmp)-1;

        __Set(32+0x0F,(u8)((FPGA_OS_Size-1) & 0x00FF));            //send OS factor to FPGA                     
        __Set(32+0x10,(u8)((FPGA_OS_Size-1) >> 8));                                    
        __Set_Param(8,0x96);                         
        __Set_Param(9,0x00);                                       //normal 150 sample pre-sampling depth
        __Set(T_BASE_PSC,FPGA_TbaseOS[i].Psc);                     //FPGA based oversampling  
        __Set(T_BASE_ARR,FPGA_ARR);                                         

        if(XposRef>150)XposRef=150;
...
u32 UTmp;
  ...
  if((OSBufferLogic())&&((FPGAsubVer==0)||(FPGAosFlag==0))){if(_T_base<16) {a*=8;b*=8;}else{a*=6;b*=6;}}                                                                   
   ...
   if(_Tr_kind==8){
     if((Sweep)&&(_Kind<5)){
       ...
       if((OSBufferLogic())&&((FPGAsubVer==0)||(FPGAosFlag==0))){if(_T_base<16)UTmp*=8;else UTmp*=6;}                                                                   
       SetTthreshold(UTmp);
     ...
     if(_T1 > _T2)  SetTthreshold(a-b);    
     else           SetTthreshold(b-a); 	 
...
if(FPGAver==0){                                   //do not use with revised FPGA
 if((_T_base>18)||(OSBufferLogic())){             //use time based triggering for OS and fastest timebases rather than edge or level trig
   if((TriggerType==0)||(TriggerType==2)){        //more reliable for random, occasional triggers or noisy OS waveforms
     ...
     SetTthreshold(0);                            //setting time to exceed at 0 turns these into an edge or level trigger
 ...
 if(service){                                     //in regular trigger modes
  ...
  if((OSBufferLogic())&&((FPGAsubVer==0)||(FPGAosFlag==0))){
...
void Process(void)
{ 
  s16 i, j = 0, k, V[8] = {0},a,b, n = 0,OsA=0,OsB=0,OsA_=0,OsB_=0;
  ...
  if((OsBufferLogicFlag)&&(FPGAosFlag==0)){                                               
    Xtend=119;                                                          //provides 512 samples for FFT 
  }else{
     if ((((_Mode==NORH)||(_Mode==NORHLD)||(_Mode==NORC))&&(!OsBufferLogicFlag)&&(_T_base < 10)&&(Options&1))||(_4_source>9)){
  ...
  if((OsBufferLogicFlag)&&((FPGAsubVer==0)||(FPGAosFlag==0))){
    k=StartOffset;
    WindowPosition=0;
  }else{
    ...
    if((OsBufferLogicFlag)&&(FPGAsubVer)&&(FPGAosFlag))Xtend+=WindowPosition;
  }



    if(AutoSetFlag){                                       //autoset uses cursor defined function to filter out transients
      if(AutoSequence==0){
        Tstart=200;
    ...
    if((((_T_base>10)|| (  (_T_base>9)&&    ( (FrameMode>0)||((FrameMode==0)&&(_Mode==NORHLD)) )    
	  ...
	  if(i == 4) {								// used for auto trigger
		...
		// FFT ///////////////////////////////////////  
           ...
           if((_4_source>10)&&(i>=WindowPosition)&&(i < (FFTSize+WindowPosition))&&(_Mode!=X_Y)&&    ((EnableFFT==0)||(_T_base>7)||(_Mode==SCAN)||(_State.Value==2))    ){
...
//if(FlagMeter>0){	//could be used to speed up display refresh rate when meters are turned off?
      if (((CurDefTime==0)&&(i>3)&&(i<bag_max_buf))||((i>start)&&(i<finish))){		//for voltage measurements, only for analog channels
        ...
        if(OsBufferLogicFlag){
          ...
          if((FPGAsubVer)&&(FPGAosFlag)){                                   //FPGA based oversampling RMS calc
      ...
      if((Title[TRACK2][COUPLE].Value==2)&&(_2_source)){
        ...
        while(j > 0 ){
          if ((Options&2)&&(_T_base<17)){               // prevent interpolate routine from interfering with non-interpolated time bases
            ...
            if((_T_base<20)&&(OsBufferLogicFlag)&&(FPGAsubVer)&&(FPGAosFlag)){
              Send_Data( V[A_]+((V[A]-V[A_])*(1024 - j))/1024, // the current CH_A point interpolation
                         ...
                         OsA_+((OsA-OsA_)*(1024 - j))/1024,                          
                         OsB_+((OsB-OsB_)*(1024 - j))/1024,
                         n++);
            }else{
              Send_Data( V[A_]+((V[A]-V[A_])*(1024 - j))/1024, // the current CH_A point interpolation
                         V[B_]+((V[B]-V[B_])*(1024 - j))/1024, // the current CH_B point interpolation
 ...
 if(!(OsBufferLogicFlag)){
   ...
   for(jj=0; jj<(discard+Tmp); jj++){                           	
    ...
    //if((_T_base<8)&&(OsBufferLogicFlag)&&(FPGAsubVer)&&(FPGAosFlag)){     //On the premise other modes can fall through at least once
      if(_T_base<8){                                                        //On the premise they can't always, and can benefit from this
...
{ 
static u8 FreeRunOffStatus=0;
u32 waste=0;
if((Update)&&(_Mode==NORHLD))UpdateCursorMeter();

if((OsBufferLogicFlag)&&(FPGAsubVer)&&(FPGAosFlag)){                   //force FPGA freerun mode when using FPGA OS 
  if((FreeRunOffStatus==0)&&(FreeRunEnable==0))FreeRunOffStatus=1;     //store non OS freerun enable status before engaging
  FreeRunEnable=1;                                                     //Turn ON freerun in FPGA OS mode by default, stays on
}else if(FreeRunOffStatus){                                            //return to previous freerun status on exiting FPGA OS
  FreeRunOffStatus=0;                                                  //reset memory    
  FreeRunEnable=0;                                                     //turn off if it was off before entering OS mode
} 

freerun=0;
if((__Get(FIFO_START)!=0)||(_Mode!=AUTO)||(FreeRunEnable==0)){
    __Set(32+14, 0);
  JComp=1;                                                             //Set FPGA FreeRun off
}
  ...
  {  
     ...
     case AUTO:
      ...
      if((__Get(FIFO_START)!=0)||(_State.Value==2)){
        ...
        if ((_T_base > 8)||((OsBufferLogicFlag)&&(FPGAosFlag==0)&&(_T_base > 7))){
          freerun=1;
          if((FreeRunEnable)&&(FPGAver>0)){
            __Set(32+14, 1 );                                      
            JComp=0;
          } 
          entryflag=0;
  ...
  u8  OSFactor=8;
	    ...
	    if((service)&&(!ChartLogic())){                                      
              ...
              if((OsBufferLogicFlag)&&(CalibrateMode==0)){
                ...
                if((FPGAsubVer)&&(FPGAosFlag)){                                 //FPGA based oversampling

                  OsTransferCompleteFlag=0;
                  Transfer=__Read_FIFO();                                       //Empty flag has been checked before calling here
                  if(Transfer & 0x00010000){                                    //If bit 16 set, load hi, finishing up pair, then load values
                    hiA=Transfer & 0x00FF;
                    hiB=(Transfer >> 8)& 0x00FF;
                  }else{                                                        //Otherwise read lo/hi pair, if hi on empty buffer loop again
                    loA=Transfer & 0x00FF;
                    loB=(Transfer >> 8)& 0x00FF;
                    if(__Get(FIFO_EMPTY)==0)Transfer=__Read_FIFO();else return;
                    hiA=Transfer & 0x00FF;                                      //If readable finish and load
                    hiB=(Transfer >> 8)& 0x00FF;
                  }  
                ...
                if(FPGAsubVer){                                              
                  ...
                  if(FPGAosFlag){
                    OsFFTData[0][i]=(hiA+loA)/2;
                    OsFFTData[1][i]=(hiB+loB)/2;
                    RMSdata[0][i]=OsFFTData[0][i];
                    RMSdata[1][i]=OsFFTData[1][i];
                  }else{
...
u8 TriggerModeLogic(void){  
   return(((Title[7][1].Value>3)&&(Title[7][1].Value<8))||((_T_base>18)&&(FPGAver==0))||((Sweep>0)&&(_Kind<5)&&(Title[7][1].Value==8)));
}

u8 OSBufferLogic(void){
  return(((_T_base<17)||((_T_base<20)&&(FPGAsubVer)&&(FPGAosFlag)))&&(OSBuffer)&&
          (!(((Title[0][1].Value==2)&&(_1_source))||((Title[1][1].Value==2)&&(_2_source))))&&
...
void SetTthreshold(u16 Time){
u32 UTmp=Time*FPGA_OS_Size;

      if((OSBufferLogic())&&(FPGAsubVer)&&(FPGAosFlag)){
        __Set(T_THRESHOLD,(u16)(UTmp&0x0000FFFF));           
        __Set(32+0x11,(u8)(UTmp>>16)&0x000000FF);            
        __Set(32+0x12,(u8)(UTmp>>24)&0x000000FF);                             
      }else{
        __Set(T_THRESHOLD,Time);          //bits 0-15
        if(FPGAsubVer){
          __Set(32+0x11,0);               //bits 16-23
          __Set(32+0x12,0);               //bits 24-31                  
        }
      }
}
